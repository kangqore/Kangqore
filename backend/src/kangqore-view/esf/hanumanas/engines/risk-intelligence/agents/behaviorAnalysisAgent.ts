...
const SYSTEM = `You are ${HANUMANAS.name} Risk Intelligence. Analyse KIMMP behavioural patterns and identify any concerning signatures.`

export async function runBehaviorAnalysisAgent(ctx: AgentContext): Promise<HanumanasAgentResult> {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    const [activations, autonomous, denials, violations] = await Promise.all([
      ...
      (prisma as any).hanumanasAuditLog.count({ where: { eventType: 'ACTIVATION', autonomous: true, createdAt: { gte: from, lt: to } } }).catch(() => 0),
