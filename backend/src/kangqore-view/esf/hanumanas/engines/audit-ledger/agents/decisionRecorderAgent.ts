...
export async function runDecisionRecorderAgent(ctx: AgentContext): Promise<HanumanasAgentResult> {
  ...
  const llmSummary = await callLLM(SYSTEM, `HANUMANAS Decision Recorder (24h): ${total} KIMMP ACTIVATION events. Record completeness: ${completeness}% (missing trigger: ${missTrigger}, system: ${missSystem}, priority: ${missPriority}, confidence: ${missConfidence}).\n\nWrite 2 sentences: current status and whether ADMIN action is needed.`, 300)
