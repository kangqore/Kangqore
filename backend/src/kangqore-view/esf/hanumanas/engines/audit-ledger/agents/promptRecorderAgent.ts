...
export async function runPromptRecorderAgent(ctx: AgentContext): Promise<HanumanasAgentResult> {
  ...
  const llmSummary = await callLLM(SYSTEM, `HANUMANAS Prompt Recorder (24h): ${total} KIMMP activations. With agent roster recorded: ${withAgents} (${coverage}%). Autonomous activations: ${autonomous}.\n\nWrite 2 sentences: current status and whether ADMIN action is needed.`, 300)
