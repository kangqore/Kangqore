...
// SOC2 Type II Trust Service Criteria mapped to HANUMANAS capabilities
const CRITERIA = [
...
export async function runSoc2ComplianceAgent(ctx: AgentContext): Promise<HanumanasAgentResult> {
  ...
  // Availability: HANUMANAS scheduler must have run in last 2h
  ...
  return {
    ...
    summary:  `SOC2 Type II: ${evidenced}/${CRITERIA.length} trust service criteria evidenced (30d). Score: ${score}%.${!schedulerActive ? ' WARNING: HANUMANAS scheduler silent >2h.' : ''}`,
    findings: [
      ...criteriaResults.map(c => `${c.id} ${c.name}: ${c.evidenced ? `✓ ${c.count} events` : '✗ NO EVIDENCE'}`),
      `Continuous monitoring (HANUMANAS): ${schedulerActive > 0 ? `✓ Active (${schedulerActive} runs)` : '✗ SILENT >2h'}`,
    ],
