// ---------------------------------------------------------------------------
// AEGIS Agent Registration — all 80 agents across 10 engines.
//
...
registerAgent({ id: 'audit.logger',            name: 'Activity Logger Agent',      engine: 'AUDIT_LEDGER', phase: 2, usesLLM: false, triggers: ['schedule.1h'],  description: 'Checks all 6 AEGIS event types have ledger entries — flags logging gaps',                    run: runLoggerAgent              })
...
registerAgent({ id: 'audit.compliance-evidence',name:'Compliance Evidence Agent',  engine: 'AUDIT_LEDGER', phase: 2, usesLLM: true,  triggers: ['schedule.24h'], description: 'LLM-compiled audit evidence coverage report across all 6 AEGIS event types',                    run: runComplianceEvidenceAgent  })
...
registerAgent({ id: 'compliance.iso',                name: 'ISO Compliance Agent',           engine: 'TRUST_COMPLIANCE', phase: 4, usesLLM: false, triggers: ['schedule.24h'], description: 'ISO 27001/42001 control coverage check across 6 AEGIS event domains — WARN if any gap',         run: runIsoComplianceAgent       })
...
registerAgent({ id: 'compliance.evidence',           name: 'Evidence Collection Agent',      engine: 'TRUST_COMPLIANCE', phase: 4, usesLLM: true,  triggers: ['schedule.24h'], description: 'LLM-compiled audit evidence package summary across all 6 AEGIS event types (30d)',                run: runEvidenceCollectionAgent  })
...
registerAgent({ id: 'risk.operational-risk',    name: 'Operational Risk Agent',          engine: 'RISK_INTELLIGENCE', phase: 4, usesLLM: false, triggers: ['schedule.6h'],  description: 'CRITICAL if AEGIS scheduler silent >2h; WARN if any of 10 engines silent >24h',              run: runOperationalRiskAgent     })
