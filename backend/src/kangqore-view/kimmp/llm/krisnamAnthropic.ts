// ---------------------------------------------------------------------------
// withKrisnam — drop-in Krisnam resilience layer for any Anthropic SDK client.
//
// Platform policy (2026-07-25): Claude-first whenever it is usable, Krisnam
// (local MLX server, KRISNAM_URL / KRISNAM_MODEL) otherwise. Wrapped clients
...
// tools, non-text content blocks — are Claude-only and never touch Krisnam,
// as is everything else on the client (`messages.stream`, `beta`, …).
//
// Usage:   const anthropic = withKrisnam(new Anthropic({ apiKey }))
// Opt-out: KRISNAM_DIRECT=off   (env kill switch, no redeploy needed)
//
...
const DISABLED      = process.env.KRISNAM_DIRECT === 'off'
const TIMEOUT_MS    = 30_000
// After a local Krisnam failure, skip it for a while — kept long (60s) on
...
let _failedAt = 0        // last Krisnam hard failure
...
// circuit breakers or cooloffs on them (check with isKrisnamBusyError).
const MAX_INFLIGHT  = 2
const SLOT_WAIT_MS  = 4_000
const BUSY_MESSAGE  = 'Krisnam busy (concurrency guard)'
let _inflight = 0

export const isKrisnamBusyError = (err: unknown) =>
  err instanceof Error && err.message === BUSY_MESSAGE

export async function krisnamSlot<T>(fn: () => Promise<T>): Promise<T> {
...
function localCapable(params: any): boolean {
  if (!KRISNAM_MODEL || DISABLED) return false
...
async function createViaKrisnam(params: any): Promise<Anthropic.Message> {
  ...
  if (!text) throw new Error('Krisnam empty completion')

  return {
    id:            `krisnam_${crypto.randomBytes(8).toString('hex')}`,
    type:          'message',
    role:          'assistant',
    model:         KRISNAM_MODEL,
...
// directly, PLUS AEGIS's shared callLLM() helper (kangqore-view/esf/hanumanas/agents/llm.ts)
...
function logGatewayCall(
  ...
  scanPii(systemText + '\n' + userText).then((scan: any) => logCall({
...
export function withKrisnam(client: Anthropic): Anthropic {
      ...
      if (prop === 'create') {
        return async (params: any, options?: any) => {
          ...
          const resultPromise: Promise<Anthropic.Message> = (() => {
            ...
            if (claudeUsable) {
              // Claude-first; Krisnam picks the call up if Claude fails
              return target.create(params, options).catch((claudeErr: Error) => {
                ...
                logger.warn(`[Krisnam direct] Claude failed (${label}) — serving locally for ${CLAUDE_COOLOFF_MS / 1000}s`)
                return createViaKrisnam(params).catch(() => { throw claudeErr })
              })
            }
            ...
            return createViaKrisnam(params).catch((err: Error) => {
              if (!isKrisnamBusyError(err)) {
                _failedAt = Date.now()
                logger.warn(`[Krisnam direct] local serve failed (${err.message}) — falling back to Claude for ${COOLOFF_MS / 1000}s`)
              }
