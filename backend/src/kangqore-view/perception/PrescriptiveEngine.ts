...
import { IntelligenceSignalEngine } from '../awareness/IntelligenceSignalEngine'
...
async function fromDescriptive(): Promise<RecommendationInput[]> {
  ...
  for (const p of snap.projects.filter((p: any) => p.severity === 'CRITICAL' || p.flag === 'OVERDUE')) {
    recs.push({
      priority: 1,
      type: 'ESCALATE_ISSUE',
  ...
  for (const c of snap.clients.filter((c: any) => c.severity !== 'LOW')) {
    recs.push({
  ...
  for (const t of snap.teams.filter((t: any) => t.severity !== 'LOW')) {
    recs.push({
      priority: 2,
  ...
  for (const s of snap.sla.filter((s: any) => s.severity === 'CRITICAL' || s.breached)) {
    recs.push({
      priority: 1,
      type: 'ESCALATE_ISSUE',
...
async function fromPredictive(): Promise<RecommendationInput[]> {
  ...
  for (const d of snap.deadlinePredictions.filter((d: any) => d.severity !== 'LOW')) {
  ...
  for (const c of snap.churnPredictions.filter((c: any) => c.severity !== 'LOW')) {
    recs.push({
      priority: c.severity === 'CRITICAL' ? 1 : 2,
  ...
  for (const o of snap.overloadPredictions.filter((o: any) => o.severity !== 'LOW')) {
    recs.push({
      priority: 2,
      type: 'ASSIGN_BACKUP',
