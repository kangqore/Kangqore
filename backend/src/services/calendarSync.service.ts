...
function googleOAuthClient() {
  return new google.auth.OAuth2(
    ...
    process.env.GOOGLE_REDIRECT_URI || `${process.env.BACKEND_URL || 'http://localhost:5050'}/api/scheduling/calendar-integrations/callback/google`
  );
}
...
export class CalendarSyncService {
  ...
  private static async refreshGoogleToken(integration: any) {
    const oauth2 = googleOAuthClient();
  ...
  static async exportEventToGoogle(integration: any, eventDetails: {
    title: string; description?: string; startTime: Date; endTime: Date; joinUrl?: string;
  }, requestMeetLink = false): Promise<string | undefined> {
    try {
      const oauth2 = googleOAuthClient();
      let accessToken = integration.accessToken;
      if (new Date(integration.expiresAt) <= new Date()) {
  ...
  private static async getOutlookAccessToken(integration: any): Promise<string> {
    ...
    await prisma.calendarIntegration.update({
      where: { id: integration.id },
      data: { accessToken: result.accessToken, expiresAt }
    });
  ...
  private static async getOutlookBusySlots(integration: any, start: Date, end: Date) {
    try {
      ...
      const items: any[] = data.value?.[0]?.scheduleItems || [];
      return items
  ...
  static async exportEventToOutlook(integration: any, eventDetails: {
    title: string; description?: string; startTime: Date; endTime: Date; joinUrl?: string;
  }) {
    try {
      const token = await this.getOutlookAccessToken(integration);
      ...
      await fetch('https://graph.microsoft.com/v1.0/me/events', {
        ...
        body: JSON.stringify({
          ...
          ...(eventDetails.joinUrl && {
            onlineMeeting: { joinUrl: eventDetails.joinUrl }
          })
        })
      });
    } catch (err) {
  ...
  static async getExternalBusySlots(userId: string, startDate: Date, endDate: Date) {
    ...
    for (const integration of integrations) {
      if (integration.provider === 'google') {
        busySlots.push(...await this.getGoogleBusySlots(integration, startDate, endDate));
      } else if (integration.provider === 'outlook') {
        busySlots.push(...await this.getOutlookBusySlots(integration, startDate, endDate));
      }
    }
  ...
  static async exportEvent(userId: string, eventDetails: {
    title: string; description?: string; startTime: Date; endTime: Date; joinUrl?: string;
  }, requestMeetLink = false): Promise<string | undefined> {
    const integrations = await prisma.calendarIntegration.findMany({
      where: { userId, syncStatus: 'ACTIVE' }
