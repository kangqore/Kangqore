    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: 'Overdue',        value: fmtK(totalOverdue), bg: totalOverdue > 0 ? 'bg-red-50 border-red-100' : 'bg-slate-50 border-slate-100', text: totalOverdue > 0 ? 'text-red-700' : 'text-slate-500' },
        ].map(c => (
              ...
              <div key={p.id} className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-sm font-bold text-slate-900">{fmtK(p.totalEarned)}</span>
                  </div>
          ...
          <tbody className="divide-y divide-slate-50">
            {[...payments].sort((a,b) => b.issuedDate.localeCompare(a.issuedDate)).map(p => (
              <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                <td className="px-5 py-3 font-mono text-xs text-slate-400">{p.invoiceNumber}</td>
                <td className="px-5 py-3 text-slate-700 font-medium text-xs">{p.partnerName.split(' ')[0]}</td>
                <td className="px-5 py-3 text-slate-600 max-w-[200px] truncate">{p.description}</td>
                <td className="px-5 py-3 font-bold text-slate-900">{fmt(p.amount)}</td>
                <td className="px-5 py-3 text-xs text-slate-400">{new Date(p.issuedDate).toLocaleDateString('en-GB',{day:'numeric',month:'short'})}</td>
                <td className="px-5 py-3 text-xs text-slate-400">{new Date(p.dueDate).toLocaleDateString('en-GB',{day:'numeric',month:'short'})}</td>
