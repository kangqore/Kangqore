...
const STEP_TYPE_CONFIG: Record<string, { label: string; color: string; Icon: React.FC<{ className?: string }> }> = {
  ...
  delay:        { label: 'Wait',         color: 'bg-slate-900 border-os-border text-slate-300', Icon: ({ className }) => <Clock         className={className ?? 'w-3.5 h-3.5'} /> },
...
function StepNode({ step, isSelected, onClick }: { step: WorkflowStep; isSelected: boolean; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-sm font-semibold mt-1.5 text-white">{step.name}</p>
        ...
        <CardBody className="p-0">
          <div className="divide-y divide-[#2E2854]">
            {workflows.map(w => (
              ...
              >
                <div className="w-7 h-7 rounded-lg bg-os-s1 flex items-center justify-center flex-shrink-0">
      ...
      <div className="lg:col-span-2 space-y-5">
        <Card>
          ...
          <CardBody className="space-y-4">
            {/* Trigger */}
            <div className="flex items-center gap-3 p-3 bg-slate-900 rounded-xl border border-os-border">
              <div className="w-8 h-8 rounded-lg bg-slate-700 flex items-center justify-center">
                <Zap className="w-4 h-4 text-white" />
              </div>
              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide">Trigger</p>
                <p className="text-sm font-semibold text-white">{wf.triggerConfig}</p>
              </div>
            </div>
              ...
              {wf.steps.map((step, i) => (
                <div key={step.id} className="flex items-start gap-2">
                  <div className="flex flex-col items-center gap-0 mt-4">
                    <span className="text-xs text-slate-500 font-mono w-5 text-center">{i + 1}</span>
            ...
            <CardBody className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="bg-slate-900 rounded-xl p-3">
                  ...
                  <p className="text-sm font-semibold text-white">Step {step.order}</p>
                </div>
              </div>
              <div className="bg-slate-900 rounded-xl p-3">
