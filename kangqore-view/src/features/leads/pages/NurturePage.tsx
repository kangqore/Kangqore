...
const STEP_COLOR: Record<string, string> = {
  ...
  linkedin: 'bg-[#2564ea]/10 text-[#2564ea]', wait: 'bg-slate-100 text-slate-400',
}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-xl font-bold text-slate-900">Nurture Sequences</h2>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold px-3 py-1.5 rounded-xl bg-slate-50 text-slate-600 border border-slate-200">{completed} completed</span>
        </div>
      </div>
              ...
              <div className="flex items-start justify-between gap-3 mb-4">
                <div>
                  <div className="flex items-center gap-2 mb-0.5">
                    <h3 className="font-semibold text-slate-900">{seq.leadCompany}</h3>
                    <Badge variant={STATUS_VARIANT[seq.status]} dot size="sm">{seq.status}</Badge>
                  </div>
                  <p className="text-xs text-slate-400">{seq.sequenceName}</p>
                  {lead && <p className="text-xs text-slate-500 mt-0.5">{lead.contactName} · {lead.contactRole}</p>}
                </div>
                <div className="text-right flex-shrink-0">
                  <p className="text-xs text-slate-400">Step {doneSteps}/{totalSteps}</p>
                ...
                {seq.steps.map((step, i) => {
                  ...
                  return (
                    <div key={step.step} className={`flex items-start gap-3 p-2.5 rounded-xl transition-colors ${isNext ? 'bg-[#2564ea]/5 border border-[#2564ea]/15' : ''}`}>
                      ...
                      {/* Step icon */}
                      <div className={`w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0 ${step.completed ? 'bg-slate-100 text-slate-400' : STEP_COLOR[step.type]}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2">
                          <p className={`text-xs font-medium ${step.completed ? 'text-slate-400 line-through' : isNext ? 'text-slate-900' : 'text-slate-600'}`}>
                            {step.title}
                          </p>
                          {isNext && (
            ...
            .map(lead => (
              <div key={lead.id} className="flex items-center gap-3 py-2 border-b border-slate-50 last:border-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-400">{lead.contactName} · {lead.stage}</p>
                </div>
                ...
                <span className="text-xs text-slate-400">£{(lead.value/1000).toFixed(0)}k</span>
              </div>
            ))}
        </div>
      </Card>
