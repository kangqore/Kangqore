    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <span className="text-sm font-bold text-slate-900">{fmt(balance)}</span>
        </CardHeader>
      ...
      <Card>
        ...
        <div className="space-y-3">
          {projectFinancials.map(p => {
            ...
            return (
              <div key={p.projectId} className="flex items-center gap-4 py-2 border-b border-slate-50 last:border-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-6 text-xs text-slate-400">
                    <span>Budget <strong className="text-slate-700">{fmt(p.budget)}</strong></span>
                    <span>Spent <strong className="text-slate-700">{fmt(p.spent)}</strong></span>
                    <span>Invoiced <strong className="text-slate-700">{fmt(p.invoiced)}</strong></span>
                    <span>Collected <strong className="text-slate-700">{fmt(p.collected)}</strong></span>
                  </div>
                </div>
