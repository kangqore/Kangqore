  ...
  return (
    <div className="space-y-6 max-w-3xl" id="executive-report">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-slate-900">Executive Report</h2>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <Card className="text-center py-4">
          ...
          <div className="text-xs text-slate-400 mt-0.5">On track</div>
        </Card>
        <Card className="text-center py-4">
          <div className="text-2xl font-bold text-amber-600">{atRisk}</div>
          <div className="text-xs text-slate-400 mt-0.5">At risk</div>
        </Card>
        <Card className="text-center py-4">
          <div className={`text-2xl font-bold ${openRisks > 0 ? 'text-red-600' : 'text-slate-400'}`}>{openRisks}</div>
          <div className="text-xs text-slate-400 mt-0.5">Open risks</div>
        </Card>
      </div>
          ...
          {projects.map(p => (
            <div key={p.id}>
              <div className="flex items-start justify-between gap-2 mb-2">
                ...
                <span className="text-sm font-bold text-slate-700 flex-shrink-0">{p.progress}%</span>
              </div>
      ...
      <Card>
        ...
        <div className="grid grid-cols-3 gap-4 mt-2">
          <div className="text-center p-3 bg-slate-50 rounded-xl">
            ...
            <div className="text-lg font-bold text-slate-800">{fmt(totalInvoiced)}</div>
          ...
          {RECENT_ACTIVITY.map((item, i) => (
            <div key={item.id}>
              ...
              <div className="flex items-center gap-3 py-3">
                ...
                <span className="text-xs text-slate-400 flex-shrink-0 whitespace-nowrap">{fmtDate(item.date)}</span>
              </div>
            </div>
          ))}
        </div>
