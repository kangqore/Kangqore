          ...
          {actionGroups.map(g => (
            <button key={g.key} onClick={() => setGroupFilter(g.key)}
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold capitalize transition-all ${
                groupFilter === g.key ? 'bg-[#2564ea] text-white' : 'bg-white border border-slate-200 text-slate-500 hover:border-blue-300'
              }`}>
              {g.label}
            </button>
          ))}
        </div>
        <span className="ml-auto text-sm text-slate-400">{visible.length} events</span>
      </div>
        ...
        {Object.entries(grouped).map(([day, logs]) => (
          <div key={day}>
            <div className="flex items-center gap-3 mb-3">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest whitespace-nowrap">{day}</p>
            ...
            <Card padding="none">
              <ul className="divide-y divide-slate-50">
                {logs.map(log => {
                  ...
                  return (
                    <li key={log.id} className="flex items-start gap-4 px-5 py-4 hover:bg-slate-50 transition-colors">
                      {/* Action icon */}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0">
                            ...
                            <p className="text-sm font-medium text-slate-800 truncate">{log.resource}</p>
                          </div>
                        ...
                        {log.user && (
                          <div className="flex items-center gap-2 mt-2">
                            ...
                            {log.user.company && (
                              <span className="text-xs text-slate-400">· {log.user.company}</span>
                            )}
                          </div>
                        )}
                      </div>
