    ...
    const fetchContent = async () => {
      try {
        ...
        const [blogsRes, casesRes] = await Promise.all([
          fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:5050'}/api/content?type=blog&limit=3`),
          fetch(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:5050'}/api/content?type=case_study&limit=3`)
        ]);
