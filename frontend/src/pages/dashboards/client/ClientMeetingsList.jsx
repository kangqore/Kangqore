...
const ClientMeetingsList = ({ isTabContent = false }) => {
  ...
  const { data: meetings, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const apiUrl = `${process.env.REACT_APP_BACKEND_URL || 'http://localhost:5050'}/api/meetings/client`;
                        ...
                        <div className="flex gap-3">
                          <button 
                            onClick={async () => {
                              try {
                                const token = localStorage.getItem('token');
                                await axios.post(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:5050'}/api/consultations/${selectedMeeting.id}/action-reschedule`, {
                                  action: 'CONFIRM',
