...
const AdminInvestorEmails = ({ embedded = false, folder = 'inbox' }) => {
  ...
  const fetchConversations = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`${BACKEND_URL}/api/admin/investor-emails`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setConversations(res.data.conversations || []);
    } catch (error) {
      console.error('Error fetching investor conversations:', error);
    } finally {
      setLoading(false);
    }
  ...
  const loadThread = async (investorId) => {
    setThreadLoading(true);
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`${BACKEND_URL}/api/admin/investor-emails/${investorId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setThread(res.data.emails || []);
      setSelectedInvestor(res.data.user || res.data.investor);
    } catch (error) {
  ...
  const handleReply = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.post(`${BACKEND_URL}/api/admin/investor-emails/reply`, {
      ...
      setAttachments([]);
      setReplyMode(false);
      setSuccessMessage('✅ Reply sent to investor!');
  ...
  const content = (
    <>
      ...
      <div className={`bg-white dark:bg-gray-900 dark:border-gray-800 rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex ${embedded ? 'h-[calc(100vh-280px)]' : 'h-[calc(100vh-200px)] min-h-[600px]'}`}>
        <div className={`${selectedInvestor ? 'hidden md:block md:w-1/3' : 'w-full'} border-r border-gray-100 flex flex-col`}>
            ...
            {filteredConversations.map(conv => (
              <button
                ...
                className={`w-full p-4 text-left border-b border-gray-50 hover:bg-gray-50 dark:bg-gray-800 dark:border-gray-700 transition-colors ${
                  selectedInvestor?.id === conv.user?.id ? 'bg-purple-50 border-l-4 border-l-purple-600' : ''
                }`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-gray-500 flex items-center gap-1 mb-1">
                      <Building2 className="w-3 h-3" /> {conv.user?.company || 'Investor'}
                    </p>
                    <p className="text-sm text-gray-600 dark:text-gray-400 truncate">{conv.lastMessage?.body?.substring(0, 50)}...</p>
                  </div>
                  {conv.unreadCount > 0 && (
                    <span className="bg-purple-500 text-white text-xs px-2 py-0.5 rounded-full">{conv.unreadCount}</span>
                  )}
                </div>
              </button>
            ))}
          ...
          <div className="flex-1 flex flex-col">
            <div className="p-4 border-b border-gray-100 bg-gray-50 dark:bg-gray-800 dark:border-gray-700/50 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <button onClick={() => setSelectedInvestor(null)} className="md:hidden p-2 hover:bg-gray-100 dark:bg-[#0a0a0c] rounded-lg">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900 dark:text-white">{selectedInvestor.name}</h3>
                  <p className="text-xs text-gray-500">{selectedInvestor.email} • {selectedInvestor.company}</p>
                </div>
              </div>
                ...
                <div key={email.id} className={`flex ${email.direction === 'outbound' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[70%] ${email.direction === 'outbound' ? 'order-2' : 'order-1'}`}>
                    ...
                    <div className={`flex items-center gap-2 mt-1 ${email.direction === 'outbound' ? 'justify-end' : 'justify-start'}`}>
                      <span className="text-[10px] text-gray-400">{email.direction === 'outbound' ? 'You' : selectedInvestor.name}</span>
              ...
              {replyMode ? (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Reply to {selectedInvestor.name}</span>
                ...
                <button onClick={() => setReplyMode(true)} className="w-full py-3 bg-gray-50 dark:bg-gray-800 dark:border-gray-700 border border-gray-200 rounded-xl text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 flex items-center justify-center gap-2 text-sm font-medium">
                  ...
                  Reply to {selectedInvestor.name}
                </button>
              )}
            </div>
          </div>
  ...
  return (
    <DashboardLayout role="admin" title="Investor Emails" subtitle="Manage investor communications">
      {content}
    </DashboardLayout>
  );
...
export default AdminInvestorEmails;
