...
import { 
  ...
  MoreVertical, CheckSquare, MessageSquare, AlertTriangle, FileText,
  User, Inbox, Flag
} from 'lucide-react';
import { format } from 'date-fns';
import RichTextEditor from '../../../components/Email/RichTextEditor';

const AdminPartnerEmails = ({ embedded = false, folder = 'inbox' }) => {
  ...
  const fetchConversations = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`${BACKEND_URL}/api/admin/partner-emails`, {
      ...
      setConversations([
        {
          partner: { id: 'demo-1', name: 'John Developer', email: 'john@partner.com', company: 'TechCorp' },
          lastMessage: { subject: 'Sprint Update', body: 'Hi team...', createdAt: new Date().toISOString(), direction: 'inbound' },
          unreadCount: 2
        },
        {
          partner: { id: 'demo-2', name: 'Sarah Designer', email: 'sarah@design.co', company: 'DesignHub' },
          lastMessage: { subject: 'UI Feedback', body: 'Great work...', createdAt: new Date(Date.now() - 86400000).toISOString(), direction: 'outbound' },
          unreadCount: 0
        }
      ]);
    } finally {
  ...
  const loadThread = async (partnerId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get(`${BACKEND_URL}/api/admin/partner-emails/${partnerId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      // Demo thread
      setThread([
        { id: '1', from: 'Partner', body: 'Hello, I have a question about the project.', direction: 'inbound', createdAt: new Date(Date.now() - 3600000).toISOString() },
        { id: '2', from: 'admin@kangqore.com', body: 'Sure, happy to help! What do you need?', direction: 'outbound', createdAt: new Date(Date.now() - 1800000).toISOString() }
      ]);
    } finally {
  ...
  const handleReply = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const res = await axios.post(`${BACKEND_URL}/api/admin/partner-emails/reply`, {
      ...
      setAttachments([]);
      setReplyMode(false);
      setSuccessMessage('✅ Reply sent to partner!');
  ...
  const content = (
    <>
      ...
      <div className={`bg-white dark:bg-gray-900 dark:border-gray-800 rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex ${embedded ? 'h-[calc(100vh-280px)]' : 'h-[calc(100vh-200px)] min-h-[600px]'}`}>
        {/* Conversations List */}
        <div className={`${selectedPartner ? 'hidden md:block md:w-1/3' : 'w-full'} border-r border-gray-100 flex flex-col`}>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm font-semibold text-gray-900 dark:text-white truncate">{conv.partner?.name}</span>
                      <span className="text-xs text-gray-400">{formatDate(conv.lastMessage?.createdAt)}</span>
                    </div>
                    <p className="text-xs text-gray-500 flex items-center gap-1 mb-1">
                      <Building2 className="w-3 h-3" /> {conv.partner?.company || 'Partner'}
                    </p>
          ...
          <div className="flex-1 flex flex-col">
            ...
            <div className="p-4 border-b border-gray-100 bg-gray-50 dark:bg-gray-800 dark:border-gray-700/50 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900 dark:text-white">{selectedPartner.name}</h3>
                  <p className="text-xs text-gray-500">{selectedPartner.email} • {selectedPartner.company}</p>
                </div>
              </div>
                ...
                <div key={email.id} className={`flex ${email.direction === 'outbound' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[70%] ${email.direction === 'outbound' ? 'order-2' : 'order-1'}`}>
                            ...
                            return isImage ? (
                              ...
                              <a key={idx} href={url} target="_blank" rel="noreferrer" className={`flex items-center gap-3 p-2.5 rounded-lg border transition-all ${
                                ...
                                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                                  email.direction === 'outbound' ? 'bg-white dark:bg-black/20' : 'bg-purple-100'
                                }`}>
                                  <FileText className={`w-4 h-4 ${email.direction === 'outbound' ? 'text-white' : 'text-purple-600'}`} />
                                </div>
                                <div className="flex-1 min-w-0">
                    ...
                    <div className={`flex items-center gap-2 mt-1 ${email.direction === 'outbound' ? 'justify-end' : 'justify-start'}`}>
                      <span className="text-[10px] text-gray-400">{email.direction === 'outbound' ? 'You' : selectedPartner.name}</span>
              ...
              {replyMode ? (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Reply to {selectedPartner.name}</span>
  ...
  return (
    <DashboardLayout role="admin" title="Partner Emails" subtitle="Manage partner communications">
      {content}
    </DashboardLayout>
  );
...
export default AdminPartnerEmails;
