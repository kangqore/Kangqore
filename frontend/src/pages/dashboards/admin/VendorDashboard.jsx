...
const VendorDashboard = () => {
  ...
  const { data: statsData, isLoading: loadingStats } = useQuery({
    ...
    queryFn: async () => {
      const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL || ''}/api/admin/vendors/stats`);
      return res.data;
    },
  ...
  const { data: vendors, isLoading: loadingVendors } = useQuery({
    ...
    queryFn: async () => {
      const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL || ''}/api/admin/vendors`);
      return res.data;
    },
    refetchInterval: 30000
  });
