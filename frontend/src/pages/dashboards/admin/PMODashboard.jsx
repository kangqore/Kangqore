...
const PMODashboard = () => {
  ...
  const { data: statsData, isLoading: loadingStats } = useQuery({
    queryKey: ['pmo-stats'],
    queryFn: async () => {
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:5050'}/api/admin/pmo/stats`);
      return res.data;
    }
  });
  ...
  const { data: projects, isLoading: loadingProjects } = useQuery({
    ...
    queryFn: async () => {
      const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL || 'http://localhost:5050'}/api/admin/pmo/projects`);
      return res.data;
    }
  });
