...
function StatusBadge({ status }) {
  ...
  return <span className={`text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-full ${colour}`}>{status}</span>;
}
      ...
      <div className="flex items-start justify-between gap-4 mb-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            {connected && accountId && <p className="text-xs text-gray-500 mt-0.5">{accountId}</p>}
          </div>
        </div>
        {connected && (
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <IntegrationCard
              ...
              description="Two-way sync. Blocks busy slots and exports new bookings to your Google calendar."
              connected={!!googleCal}
        ...
        <section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <IntegrationCard
              ...
              name="Zoom"
              description="Auto-create Zoom meetings on booking. Select Zoom as the video provider on any event type."
            ...
            <div className="border border-gray-200 dark:border-gray-800 rounded-2xl p-6 flex flex-col">
              ...
              <p className="text-sm text-gray-500 mb-4 flex-1">
                Auto-creates Google Meet links via Google Calendar. Connect Google Calendar above, then select "Google Meet" on your event type.
              </p>
              <div className={`flex items-center gap-2 text-sm font-bold ${googleCal ? 'text-green-600' : 'text-gray-400'}`}>
                {googleCal
                  ? <><Check className="w-4 h-4" /> Available via Google Calendar</>
                  : <><AlertCircle className="w-4 h-4" /> Connect Google Calendar first</>
                }
              </div>
            </div>
          </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 bg-white dark:bg-gray-800 p-3 rounded-xl border border-gray-200 dark:border-gray-700">
                    ...
                    <button
                      onClick={() => { navigator.clipboard.writeText(`${BACKEND_URL}/api/scheduling/feed/${user.id}`); toast({ title: 'Copied' }); }}
