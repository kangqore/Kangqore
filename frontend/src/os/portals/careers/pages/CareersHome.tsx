...
const DEPT_COLOR: Record<string, string> = {
  ...
  ops: 'bg-[#151C2F] text-slate-500', design: 'bg-pink-50 text-pink-700',
}
    ...
    <div className="flex-1 overflow-y-auto px-6 lg:px-10 py-8 space-y-10">
              ...
              <CardBody className="p-5">
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {role.requirements.slice(0, 3).map((req, i) => (
                    <span key={i} className="text-xs bg-[#151C2F] text-slate-500 px-2 py-0.5 rounded-full">{req}</span>
                  ))}
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          {PERKS.map(p => (
            <div key={p.label} className="bg-[#151C2F] border border-[#2E2854] rounded-xl p-4">
