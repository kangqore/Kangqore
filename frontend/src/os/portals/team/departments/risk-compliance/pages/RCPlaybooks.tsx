...
const TEMPLATES: PlaybookTemplate[] = [
  {
    ...
    lastRun: '2mo ago',
    steps: [
      { id: 's1', label: 'Detect & Classify',    instructions: 'Confirm the breach. Classify severity (P1 regulatory notification required / P2 internal / P3 minor). Log in HANUMANAS audit trail immediately. Identify affected data categories and estimated volume.', assignee: 'Maya Nair', sla: '1h of detection', status: 'PENDING' },
      { id: 's2', label: 'Contain',              instructions: 'Isolate affected systems. Revoke compromised credentials. Engage IT to block exfiltration vectors. Preserve forensic evidence — do not wipe systems without legal approval.', assignee: 'IT / Maya Nair', sla: '2h of detection', status: 'PENDING', kimmHint: 'KIMMP can cross-reference HANUMANAS access logs to identify the blast radius within 5 minutes — trigger via the HANUMANAS Intelligence Registry.' },
      ...
      { id: 's4', label: 'Remediate',            instructions: 'Close the attack vector. Apply patches, rotate credentials, update firewall rules. Document all actions in HANUMANAS. Run a secondary scan to confirm containment. Update risk register.', assignee: 'IT / Maya Nair', sla: '24h of containment', status: 'PENDING', kimmHint: 'Remediation steps that are undocumented in HANUMANAS will not satisfy ICO investigation evidence requirements — log every action with timestamp and operator.' },
