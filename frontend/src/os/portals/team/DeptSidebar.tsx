    ...
    >
      ...
      >
        {config.navGroups.map(group => {
          // Filter items based on user RBAC
          const visibleItems = group.items.filter(item => {
            if (item.requiresLead && !user?.isDepartmentLead) return false
            if (item.requiresHr && (!user?.isDepartmentHr && !user?.isDepartmentLead)) return false
            return true
          })
