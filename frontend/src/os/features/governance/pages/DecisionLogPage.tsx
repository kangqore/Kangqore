...
const STATUS_ICON: Record<DecisionStatus, React.ReactNode> = {
  ...
  WITHDRAWN:        <Minus       className="w-4 h-4 text-[var(--os-text-2)]"  />,
}
    ...
    >
      <div className="space-y-5">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div>
            <p className="text-xs font-semibold text-[var(--os-text-2)] mb-1.5">Priority</p>
            <Badge variant={PRIORITY_V[decision.priority]} size="md">{decision.priority}</Badge>
          </div>
          {decision.dueDate && (
            <div>
              <p className="text-xs font-semibold text-[var(--os-text-2)] mb-1.5">Decision due</p>
              <p className="text-sm font-medium text-[var(--os-text-1)]">{fmtDate(decision.dueDate)}</p>
            </div>
          )}
        </div>
        ...
        {decision.description && (
          <div>
            <p className="text-xs font-semibold text-[var(--os-text-2)] uppercase tracking-wide mb-2">Background</p>
            <p className="text-sm text-[var(--os-text-1)] leading-relaxed">{decision.description}</p>
          </div>
        )}
        ...
        {(decision.approver || decision.client || decision.risk || decision.changeRequest) && (
          <>
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              {decision.approver && (
                <div>
                  <p className="text-[var(--os-text-2)] mb-0.5">Approver</p>
                  <p className="font-medium text-[var(--os-text-1)]">{decision.approver.name}</p>
                  {decision.approver.authorityRole && <p className="text-[var(--os-text-2)]">{decision.approver.authorityRole}</p>}
                </div>
              )}
              {decision.client && (
                <div>
                  <p className="text-[var(--os-text-2)] mb-0.5">Client</p>
                  <p className="font-medium text-[var(--os-text-1)]">{decision.client.name}</p>
                </div>
              )}
              {decision.risk && (
                <div>
                  <p className="text-[var(--os-text-2)] mb-0.5">Linked risk</p>
                  <p className="font-medium text-[var(--os-text-1)]">{decision.risk.title}</p>
                </div>
              )}
              {decision.changeRequest && (
                <div>
                  <p className="text-[var(--os-text-2)] mb-0.5">Linked CR</p>
                  <p className="font-medium text-[var(--os-text-1)]">{decision.changeRequest.title}</p>
                </div>
              )}
            </div>
          </>
        )}
        <div className="text-xs text-[var(--os-text-2)]">Logged {fmtDate(decision.createdAt)}</div>
      </div>
    </EditDrawer>
  )
}
  ...
  return (
    ...
    >
      <div className="space-y-4">
        <div>
          <label className="block text-xs font-semibold text-[var(--os-text-2)] mb-1.5">Decision title *</label>
          <Input value={form.title} onChange={e => set('title', e.target.value)} placeholder="What was decided?" />
        </div>
        <div>
          <label className="block text-xs font-semibold text-[var(--os-text-2)] mb-1.5">Background</label>
        ...
        <div>
          <label className="block text-xs font-semibold text-[var(--os-text-2)] mb-1.5">Rationale *</label>
          <Textarea value={form.rationale} onChange={e => set('rationale', e.target.value)} rows={3} placeholder="Why this decision was made" />
        </div>
        <div>
          <label className="block text-xs font-semibold text-[var(--os-text-2)] mb-1.5">Trade-offs</label>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            <label className="block text-xs font-semibold text-[var(--os-text-2)] mb-1.5">Decision due</label>
            <Input type="date" value={form.dueDate} onChange={e => set('dueDate', e.target.value)} />
          </div>
          <div>
            <label className="block text-xs font-semibold text-[var(--os-text-2)] mb-1.5">Cost impact (₹)</label>
            <Input type="number" value={form.impactCost} onChange={e => set('impactCost', e.target.value)} placeholder="e.g. -5000" />
          </div>
          <div>
            <label className="block text-xs font-semibold text-[var(--os-text-2)] mb-1.5">Time impact</label>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h2 className="text-[22px] font-black tracking-tight" style={{ color: 'var(--os-text-1)' }}>Decision Log</h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <StatCard label="Total"    value={decisions.length}     icon={<CheckCircle2 className="w-5 h-5" />} iconColor="bg-[var(--os-surface-0)] text-[var(--os-text-2)]" />
        <StatCard label="Pending"  value={pending}              icon={<Clock        className="w-5 h-5" />} iconColor={pending > 0 ? 'bg-amber-100 text-amber-600' : 'bg-[var(--os-surface-0)] text-[var(--os-text-2)]'} />
        ...
        <StatCard label="Rejected" value={byStatus('REJECTED')} icon={<XCircle      className="w-5 h-5" />} iconColor={byStatus('REJECTED') > 0 ? 'bg-red-100 text-red-600' : 'bg-[var(--os-surface-0)] text-[var(--os-text-2)]'} />
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <span className="ml-auto text-sm text-[var(--os-text-2)]">{visible.length} decisions</span>
      </div>
      ...
      <Card padding="none">
        <table className="w-full text-sm">
          ...
          <StaggerTableBody className="divide-y divide-[var(--os-border)]">
            {visible.map(d => (
              <StaggerRow key={d.id} className="hover:bg-[var(--os-surface-0)] transition-colors cursor-pointer group" onClick={() => setOpenId(d.id)}>
                <td className="px-4 py-3.5 max-w-[240px]">
                  <div className="flex items-start gap-2">
                    {STATUS_ICON[d.status]}
                    <p className="text-xs font-medium text-[var(--os-text-1)] leading-snug line-clamp-2">{d.title}</p>
                  </div>
                </td>
                <td className="px-4 py-3.5 text-xs text-[var(--os-text-2)] whitespace-nowrap">{d.project?.title ?? '—'}</td>
                <td className="px-4 py-3.5 text-xs whitespace-nowrap">
                  ...
                  {d.impactTime && <span className="ml-2 text-[var(--os-text-2)]">{d.impactTime}</span>}
                </td>
                <td className="px-4 py-3.5">
                  <Badge variant={PRIORITY_V[d.priority]} size="sm">{d.priority}</Badge>
                </td>
                <td className="px-4 py-3.5 text-xs text-[var(--os-text-2)] whitespace-nowrap">{fmtDate(d.dueDate)}</td>
                <td className="px-4 py-3.5" onClick={e => e.stopPropagation()}>
                  <InlineSelect value={d.status} options={STATUS_OPTIONS} onChange={status => onPatch(d.id, { status })} dot />
                </td>
                <td className="px-4 py-3.5 text-xs text-[var(--os-text-2)] whitespace-nowrap">{fmtDate(d.createdAt)}</td>
              </StaggerRow>
            ))}
          </StaggerTableBody>
        </table>
