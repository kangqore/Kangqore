    ...
    <div className="rounded-3xl p-6" style={{ background: 'var(--os-card)', borderRadius: 'var(--os-radius-xl)' }}>
      ...
      {isLoading && (
        <div className="space-y-2">
          {[1,2,3].map(i => (
            <div key={i} className="h-16 rounded-2xl animate-pulse" style={{ background: 'var(--os-surface-0)' }} />
          ))}
        </div>
      )}
      ...
      {!isLoading && rows.length > 0 && (
        <div className="space-y-2">
          {rows.map(ex => (
            <div key={ex.id} className="flex items-start gap-3 p-3 rounded-2xl border border-[var(--os-border)] bg-[var(--os-surface-0)]">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[10px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded-2xl" style={{ background: 'rgba(127,83,249,0.12)', color: '#a78bfa' }}>
                    {ex.exampleType}
                  </span>
                  {ex.system && (
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                <button
                  ...
                  className="w-7 h-7 rounded-2xl flex items-center justify-center transition-colors hover:bg-green-500/15 text-green-500 disabled:opacity-40"
                  title="Approve"
                >
                ...
                <button
                  ...
                  disabled={rate.isPending}
                  className="w-7 h-7 rounded-2xl flex items-center justify-center transition-colors hover:bg-amber-500/15 text-amber-500 disabled:opacity-40"
                ...
                <button
                  ...
                  className="w-7 h-7 rounded-2xl flex items-center justify-center transition-colors hover:bg-red-500/15 text-red-500 disabled:opacity-40"
                  title="Delete"
                >
          ...
          <div className="pt-4 border-t border-[var(--os-border)]">
            ...
            <div className="grid grid-cols-2 gap-3">
              {Object.entries(bySystem).map(([sys, count]) => (
                <div key={sys} className="flex justify-between px-3 py-2 rounded-2xl bg-[var(--os-surface-0)] border border-[var(--os-border)]">
        ...
        <div className="flex flex-wrap gap-2">
          {EXAMPLE_PROMPTS.map((p, i) => (
            <button
              key={i}
              onClick={() => setPrompt(p)}
              className="text-[10px] font-semibold px-2.5 py-1 rounded-2xl transition-colors"
        ...
        <div>
          ...
          <div className="flex items-center justify-between mt-2">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-2xl text-xs font-bold transition-opacity disabled:opacity-40"
  ...
  if (isLoading && !genStatus) {
    return (
      <div className="space-y-4">
        {[1, 2, 3].map(i => (
          <div key={i} className="rounded-2xl p-6" style={{ background: '#0d1117', border: '1px solid #2E2854' }}>
      ...
      <div className="flex items-start justify-between gap-4 pb-6 mb-2 border-b border-[var(--os-border)]">
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-2xl text-xs font-bold transition-colors"
