...
function SourceCard({ source, onRun, running }: { source: ScoutSource; onRun: (name: string) => void; running: boolean }) {
  ...
  return (
    <div className={`bg-[#151C2F] border border-[#2E2854] border-l-4 ${CATEGORY_BORDER[source.signalCategory]} rounded-xl p-4 shadow-sm flex flex-col gap-3`}>
...
function JobRow({ job }: { job: ScoutJob }) {
  return (
    <div className="flex items-center gap-3 py-2.5 px-4 border-b border-[#2E2854] last:border-0 hover:bg-[#0F172A] transition-colors">
    ...
    <div className="space-y-8 max-w-5xl">
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <button
            onClick={() => refetchJobs()}
            className="ml-auto w-7 h-7 rounded-lg flex items-center justify-center text-slate-300 hover:bg-[#151C2F] hover:text-slate-300 transition-colors"
          >
