...
export function Gen2AccuracyPage() {
  ...
  return (
    <div className="space-y-5 max-w-4xl">
      ...
      {showForm && (
        <div className="rounded-xl border p-4 space-y-3" style={{ background: CARD, borderColor: BDR }}>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowForm(false)} className="px-4 py-2 rounded-xl text-xs font-bold" style={{ color: T2 }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      {/* Tenant list */}
      {isLoading ? (
        <div className="space-y-2">{[1, 2, 3].map(i => <div key={i} className="h-24 rounded-xl animate-pulse" style={{ background: SURF }} />)}</div>
      ) : tenants.length === 0 ? (
        <div className="rounded-xl p-8 text-center border" style={{ background: CARD, borderColor: BDR }}>
        ...
        <div className="space-y-3">
          {tenants.sort((a, b) => (b.gen2AccuracyPct ?? 0) - (a.gen2AccuracyPct ?? 0)).map(t => (
            <div key={t.tenantId} className="rounded-xl border p-4" style={{ background: CARD, borderColor: BDR }}>
