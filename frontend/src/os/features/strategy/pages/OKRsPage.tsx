  ...
  return (
    <div className="space-y-12 pb-16">
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 mb-2">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-[var(--os-text-1)]">OKRs — {selectedQuarter}</h2>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        {[
          ...
          return (
            <div key={s.status} className="flex items-center gap-2 px-4 py-2 bg-[var(--os-surface-0)] border border-[var(--os-border)] rounded-2xl text-xs">
              <span className="w-2.5 h-2.5 rounded-full" style={{ background: s.color }} />
              <span className="font-semibold text-[var(--os-text-2)]">{s.label}</span>
              <span className="font-bold text-[var(--os-text-1)] ml-1">{count}</span>
            </div>
          )
        })}
      </div>
      ...
      <div className="flex items-center gap-4 flex-wrap pt-2">
        ...
        <select
          ...
          className="h-10 rounded-2xl border border-[var(--os-border)] bg-[var(--os-surface-0)] text-sm text-[var(--os-text-1)] pl-4 pr-10 outline-none focus:border-blue-400"
        >
      ...
      {objectives.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <p className="text-[var(--os-text-2)] text-sm">No objectives match your filters.</p>
        </div>
      ) : (
