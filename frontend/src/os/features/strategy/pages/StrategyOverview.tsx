  ...
  return (
    <div className="space-y-12 pb-16">
      ...
      <div className="flex items-center justify-between flex-wrap gap-6 mb-2">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-[var(--os-text-1)]">Strategy Overview</h2>
          <p className="text-sm text-[var(--os-text-2)] mt-1">Company-wide strategic health across all pillars and OKRs</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
        <StatCard
          ...
          iconColor="bg-[var(--os-surface-0)] text-[var(--os-text-2)]"
          change={0}
        ...
        <StatCard
          ...
          iconColor={health.atRisk + health.behind > 1 ? 'bg-red-50 text-red-600' : 'bg-[var(--os-surface-0)] text-[var(--os-text-2)]'}
              ...
              {donutData.map(d => (
                <div key={d.name} className="flex items-center justify-between text-xs">
                  <span className="flex items-center gap-2">
                    ...
                    <span className="text-[var(--os-text-2)] font-medium">{d.name}</span>
                  </span>
                  <span className="font-bold text-[var(--os-text-1)]">{d.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-5">
            {pillars.map(p => (
              <div key={p.id} className="flex items-center gap-4">
                ...
                <span className="text-xs font-semibold text-[var(--os-text-2)] w-36 truncate">{p.name}</span>
                ...
                <span className="text-xs font-bold text-[var(--os-text-2)] w-16 text-right">
      ...
      <div>
        <h3 className="text-base font-bold text-[var(--os-text-1)] mb-5 tracking-tight">Strategic Pillars</h3>
      ...
      <Card padding="lg">
        <CardHeader className="mb-6">
          <div className="space-y-1">
            ...
            <p className="text-xs text-[var(--os-text-2)]">Currently executing workstreams</p>
          </div>
        ...
        <div className="space-y-4">
          {programs.filter(p => p.status === 'active').slice(0, 6).map(prog => (
            <div key={prog.id} className="flex items-center gap-6 py-4 border-b border-[var(--os-border)] last:border-0 last:pb-0 first:pt-0">
              <div className="w-2 h-8 rounded-full flex-shrink-0" style={{ background: prog.pillarColor }} />
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-[var(--os-text-1)] truncate">{prog.name}</p>
                <p className="text-xs text-[var(--os-text-2)] mt-0.5">{prog.pillarName}</p>
              </div>
              <div className="w-36">
                <Progress value={prog.progress} size="sm" color="brand" />
                <p className="text-[10px] text-[var(--os-text-2)] mt-1.5 text-right font-semibold">{prog.progress}%</p>
              </div>
