...
const TREND_ICON: Record<string, React.ReactNode> = {
  ...
  STABLE:     <Minus         className="w-3.5 h-3.5 text-[var(--os-text-2)]" />,
    ...
    >
      <div className="space-y-5">
        ...
        <div className="flex items-center gap-4 flex-wrap">
          ...
          <div>
            <p className="text-xs font-semibold text-[var(--os-text-2)] mb-1.5">Severity</p>
            <Badge variant={SEV_VARIANT[risk.severity]} size="md">{risk.severity}</Badge>
          </div>
          <div>
            <p className="text-xs font-semibold text-[var(--os-text-2)] mb-1.5">Risk score</p>
          ...
          {risk.trend && (
            <div>
              <p className="text-xs font-semibold text-[var(--os-text-2)] mb-1.5">Trend</p>
              <div className="flex items-center gap-1.5">
                {TREND_ICON[risk.trend]}
                <span className="text-xs text-[var(--os-text-2)] capitalize">{risk.trend.toLowerCase()}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p className="text-xs font-semibold text-[var(--os-text-2)] mb-1">Probability</p>
            <p className="text-sm font-medium text-[var(--os-text-1)] capitalize">{risk.probability?.toLowerCase() ?? '—'}</p>
          </div>
          <div>
            <p className="text-xs font-semibold text-[var(--os-text-2)] mb-1">Impact</p>
            <p className="text-sm font-medium text-[var(--os-text-1)] capitalize">{risk.impact?.toLowerCase() ?? '—'}</p>
          </div>
          {risk.owner && (
            <div>
              <p className="text-xs font-semibold text-[var(--os-text-2)] mb-1">Owner</p>
              <p className="text-sm font-medium text-[var(--os-text-1)]">{risk.owner}</p>
            </div>
          )}
          {risk.riskOwner && (
            <div>
              <p className="text-xs font-semibold text-[var(--os-text-2)] mb-1">Risk domain</p>
              <p className="text-sm font-medium text-[var(--os-text-1)] capitalize">{risk.riskOwner?.toLowerCase().replace(/_/g, ' ')}</p>
            </div>
          )}
        </div>
        ...
        {/* Description */}
        <div>
          <label className="block text-xs font-semibold text-[var(--os-text-2)] uppercase tracking-wide mb-2">Description</label>
          <Textarea
            value={description}
        ...
        {/* Mitigation */}
        <div>
          <label className="block text-xs font-semibold text-[var(--os-text-2)] uppercase tracking-wide mb-2">Mitigation plan</label>
          <Textarea
            value={mitigation}
        ...
        {/* Contingency */}
        <div>
          <label className="block text-xs font-semibold text-[var(--os-text-2)] uppercase tracking-wide mb-2">Contingency plan</label>
          <Textarea
            value={contingency}
        ...
        {risk.clientAcceptedBy && (
          <>
            ...
            <div className="p-3 bg-slate-900/40 backdrop-blur-2xl saturate-200 shadow-[inset_0_1px_0_var(--os-border),0_8px_32px_rgba(0,0,0,0.3)] ring-1 ring-white/10 border-l-4 border-l-[#00c875] border-y border-r border-[var(--os-border)] border-t-white/20 rounded-2xl">
              ...
              {risk.clientResponse && (
                <p className="text-xs text-[var(--os-text-2)] mt-1">{risk.clientResponse}</p>
              )}
            </div>
          </>
        )}
  ...
  return (
    ...
    >
      <div className="space-y-4">
        <div>
          <label className="block text-xs font-semibold text-[var(--os-text-2)] mb-1.5">Risk title *</label>
        ...
        <div>
          <label className="block text-xs font-semibold text-[var(--os-text-2)] mb-1.5">Description</label>
          <Textarea value={form.description ?? ''} onChange={e => set('description', e.target.value)} rows={3} placeholder="Full risk description" />
        </div>

        <div>
          <label className="block text-xs font-semibold text-[var(--os-text-2)] mb-1.5">Mitigation plan</label>
...
function ImpactMatrix({ risks }: { risks: Risk[] }) {
  ...
  return (
    <Card>
      <p className="text-xs font-semibold text-[var(--os-text-2)] uppercase tracking-wide mb-3">Probability × Impact matrix</p>
      <div className="overflow-x-auto">
        <table className="w-full text-xs border-collapse">
          <thead>
            <tr>
              <th className="text-left text-[var(--os-text-2)] pr-2 pb-2 font-medium w-20">Probability ↓</th>
              {IMPACTS.map(i => (
                <th key={i} className="pb-2 text-center font-semibold text-[var(--os-text-2)] capitalize px-2">{i.toLowerCase()}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {LEVELS.map(prob => (
              <tr key={prob}>
                <td className="py-1.5 pr-2 font-semibold text-[var(--os-text-2)] capitalize">{prob.toLowerCase()}</td>
                {IMPACTS.map(imp => {
...
export function RiskRegisterPage() {
  ...
  const SEV_COLOR: Record<string, string> = {
    CRITICAL: '#7c3aed', HIGH: '#e2445c', MEDIUM: '#fdab3d', LOW: '#00c875',
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h2 className="text-xl font-bold" style={{ color: 'var(--os-text-1)' }}>Risk Register</h2>
          <p className="text-sm mt-0.5" style={{ color: 'var(--os-text-2)' }}>
            {risks.length} risks · {byStatus('OPEN')} open
            {escalated > 0 && <span className="ml-2 font-medium" style={{ color: '#e2445c' }}>{escalated} escalated</span>}
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        {[
          { label: 'Total',     value: risks.length,          color: '#579bfc', icon: <Shield className="w-5 h-5" /> },
          { label: 'Open',      value: byStatus('OPEN'),      color: byStatus('OPEN') > 0 ? '#e2445c' : '#00c875', icon: <AlertTriangle className="w-5 h-5" /> },
          { label: 'Escalated', value: escalated,             color: escalated > 0 ? '#fdab3d' : '#00c875', icon: <ArrowUpCircle className="w-5 h-5" /> },
          { label: 'Mitigated', value: byStatus('MITIGATED'), color: '#579bfc', icon: <Shield className="w-5 h-5" /> },
          { label: 'Accepted',  value: byStatus('ACCEPTED'),  color: '#00c875', icon: <Shield className="w-5 h-5" /> },
        ].map(stat => (
          <div key={stat.label} className="p-4" style={{ background: 'var(--os-card)', border: '1px solid var(--os-border)', borderRadius: 12, boxShadow: 'var(--os-shadow-card)' }}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: 'var(--os-text-2)' }}>{stat.label}</span>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <span className="ml-auto text-sm text-[var(--os-text-2)]">{visible.length} risks</span>
      </div>
      ...
      <Card padding="none">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[var(--os-border)] border-t-white/20 bg-slate-900">
            ...
            {visible.map(r => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-4 py-3.5 text-xs text-[var(--os-text-2)] whitespace-nowrap">
                    {r.project?.title ?? '—'}
                  </td>
                  ...
                  <td className="px-4 py-3.5 text-xs text-[var(--os-text-2)] whitespace-nowrap">{r.owner ?? '—'}</td>
        ...
        {visible.length === 0 && (
          <div className="py-14 text-center text-sm text-[var(--os-text-2)]">No risks match your filters.</div>
        )}
      </Card>
