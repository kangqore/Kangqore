    ...
    >
      <div className="space-y-5">
        ...
        {risk.clientAcceptedBy && (
          <>
            <Divider />
            <div className="p-3 bg-[#151C2F] border-l-4 border-l-[#00c875] border-y border-r border-[#2E2854] rounded-xl">
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
        <StatCard label="Total"     value={risks.length}         icon={<Shield       className="w-5 h-5" />} iconColor="bg-[#151C2F] text-slate-300"     />
        <StatCard label="Open"      value={byStatus('OPEN')}     icon={<AlertTriangle className="w-5 h-5" />} iconColor={byStatus('OPEN') > 0 ? 'bg-red-100 text-red-600' : 'bg-[#151C2F] text-slate-300'}      />
        <StatCard label="Escalated" value={escalated}            icon={<ArrowUpCircle className="w-5 h-5" />} iconColor={escalated > 0 ? 'bg-amber-100 text-amber-600' : 'bg-[#151C2F] text-slate-300'} />
