    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl p-5 border" style={{ background: CARD, borderColor: BDR }}>
        <div className="flex items-start gap-4">
          <div className="w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(124,58,237,0.1)' }}>
        ...
        <div className="rounded-2xl p-4 border" style={{ background: 'rgba(0,200,117,0.06)', borderColor: 'rgba(16,185,129,0.2)' }}>
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 px-3 py-2 rounded-2xl border" style={{ background: SURF, borderColor: BDR }}>
      ...
      <div className="rounded-2xl border" style={{ background: CARD, borderColor: BDR }}>
        ...
        {showCreate && (
          <div className="px-5 py-4 border-b flex items-center gap-3" style={{ borderColor: BDR }}>
            <input autoFocus value={createName} onChange={e => setCreateName(e.target.value)}
              ...
              className="flex-1 px-3 py-2 text-xs rounded-2xl border focus:outline-none"
        ...
        {isLoading && (
          <div className="p-5 space-y-2">
            {[1,2].map(i => <div key={i} className="h-12 rounded-2xl animate-pulse" style={{ background: SURF }} />)}
          </div>
        )}
            ...
            {keys.map(k => (
              <div key={k.id} className="flex items-center gap-3 px-5 py-3">
                ...
                <button onClick={() => revoke.mutate(k.id)} disabled={revoke.isPending}
                  className="w-7 h-7 rounded-2xl flex items-center justify-center transition-colors disabled:opacity-40"
                  style={{ color: RED }}>
...
function AgentsTab({ qc }: { qc: ReturnType<typeof useQueryClient> }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl p-5 border" style={{ background: CARD, borderColor: BDR }}>
        <div className="flex items-start gap-4">
          <div className="w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(20,184,166,0.1)' }}>
      ...
      <div className="rounded-2xl border" style={{ background: CARD, borderColor: BDR }}>
        ...
        {showRegister && (
          <div className="px-5 py-4 border-b space-y-3" style={{ borderColor: BDR }}>
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => setShowRegister(false)} className="px-4 py-2 rounded-2xl text-xs font-bold" style={{ color: T2 }}>
                Cancel
              </button>
            </div>
          </div>
        ...
        {isLoading && (
          <div className="p-5 space-y-2">{[1,2,3].map(i => <div key={i} className="h-16 rounded-2xl animate-pulse" style={{ background: SURF }} />)}</div>
        )}
        ...
        {!isLoading && agents.map(a => (
          <div key={a.id} className="px-5 py-4 border-b" style={{ borderColor: BDR }}>
            <div className="flex items-start gap-3">
              <div className="w-8 h-8 rounded-2xl flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: STATUS_BG[a.status] ?? SURF }}>
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                <button onClick={() => ping.mutate(a.id)} disabled={ping.isPending}
                  title="Ping agent" className="w-7 h-7 rounded-2xl flex items-center justify-center disabled:opacity-40"
                  style={{ color: T2 }}>
                ...
                <button onClick={() => toggleStatus.mutate({ id: a.id, status: a.status === 'ACTIVE' ? 'PAUSED' : 'ACTIVE' })}
                  ...
                  className="w-7 h-7 rounded-2xl flex items-center justify-center disabled:opacity-40"
                ...
                <button onClick={() => deregister.mutate(a.id)} disabled={deregister.isPending}
                  title="Deregister" className="w-7 h-7 rounded-2xl flex items-center justify-center disabled:opacity-40"
                  style={{ color: RED }}>
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
              </div>
      ...
      <div className="rounded-2xl p-5 border" style={{ background: CARD, borderColor: BDR }}>
        ...
        <pre className="text-xs font-mono rounded-2xl p-4 overflow-x-auto leading-relaxed" style={{ background: SURF, borderColor: BDR, color: T2, border: `1px solid ${BDR}` }}>{`{
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl p-5 border" style={{ background: CARD, borderColor: BDR }}>
        ...
        <pre className="text-xs font-mono rounded-2xl p-4 overflow-x-auto leading-relaxed" style={{ background: SURF, border: `1px solid ${BDR}`, color: T2 }}>{`# Get latest OIS score
