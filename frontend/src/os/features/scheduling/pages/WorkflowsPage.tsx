  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-[22px] font-black tracking-tight" style={{ color: 'var(--os-text-1)' }}>Workflows</h2>
          <p className="text-sm text-[var(--os-text-2)] mt-1">Automate emails and webhooks around booking events.</p>
        </div>
        {!showForm && (
      ...
      {showForm && (
        <Card>
          <CardBody className="p-6 space-y-5">
            <h3 className="font-semibold text-[var(--os-text-1)]">Create automation</h3>
      ...
      {wfLoading && <div className="flex items-center gap-2 text-sm text-[var(--os-text-2)]"><Spinner size="sm" /> Loading…</div>}

      {!wfLoading && workflows.length === 0 && !showForm && (
        <Card><CardBody className="text-center py-12">
          <GitBranch className="w-8 h-8 text-[var(--os-text-2)] mx-auto mb-3" />
          <p className="text-sm text-[var(--os-text-2)]">No workflows yet</p>
        </CardBody></Card>
      )}
            ...
            <CardBody className="p-4">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-[var(--os-text-1)]">
                    {wf.action === 'SEND_EMAIL' ? 'Send email' : 'Call webhook'}
                    {wf.eventTypeName && <span className="text-[var(--os-text-2)]"> · {wf.eventTypeName}</span>}
                  </p>
                  <p className="text-xs text-[var(--os-text-2)] flex items-center gap-1 mt-0.5">
