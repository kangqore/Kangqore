...
export function SkillsMatrixPage() {
  ...
  return (
    <div className="space-y-5">
      ...
      <div>
        <h2 className="text-xl font-bold text-slate-900">Skills Matrix</h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <StatCard label="Single point risk" value={gapSkills}           icon={<AlertCircle  className="w-5 h-5" />} iconColor={gapSkills > 0 ? 'bg-amber-100 text-amber-600' : 'bg-slate-100 text-slate-400'} />
        <StatCard label="Team size"         value={team.length}         icon={<Circle       className="w-5 h-5" />} iconColor="bg-slate-100 text-slate-500" />
      </div>
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
              !filterSkill ? 'bg-[#2564ea] text-white' : 'bg-white border border-slate-200 text-slate-500 hover:border-blue-300'
            }`}
          >
            All skills
          </button>
          {SKILL_GAPS.map(skill => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                filterSkill === skill ? 'bg-amber-500 text-white' : 'bg-white border border-slate-200 text-slate-500 hover:border-amber-300'
              }`}
            >
              {skill}
            </button>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-slate-50">
                      <th className="text-left px-5 py-2.5 text-xs font-semibold text-slate-400 w-48">Person</th>
                        ...
                        <td className="px-5 py-3">
                          <div className="flex items-center gap-2.5">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-slate-400 truncate max-w-[120px]">{member.role}</p>
                            </div>
                          </div>
                        </td>
                        {activeSkills.map(skill => {
                            ...
                            <td key={skill} className="px-3 py-3 text-center">
                              {hasSkill ? (
                                <div className="flex justify-center">
                                  <div className="w-5 h-5 rounded-full bg-[#2564ea] flex items-center justify-center">
      ...
      {searchedTeam.length === 0 && (
        <div className="py-16 text-center text-sm text-slate-400">No team members match your search.</div>
      )}
    </div>
  )
}
