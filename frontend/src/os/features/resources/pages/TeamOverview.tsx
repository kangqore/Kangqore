  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Overloaded (≥95%)"  value={overloaded}     icon={<AlertTriangle className="w-5 h-5" />} iconColor={overloaded > 0 ? 'bg-red-100 text-red-600' : 'bg-[#151C2F] text-slate-300'} />
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {DEPTS.map(d => (
            <button key={d} onClick={() => setDept(d)} className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all capitalize ${dept === d ? 'bg-blue-600 text-white' : 'bg-[#151C2F] border border-[#2E2854] text-slate-300 hover:border-blue-300'}`}>
              {d}
            </button>
          ))}
        </div>
        ...
        {visible.map(member => {
          ...
          return (
            ...
            <Card className="hover:shadow-md transition-all duration-200 hover:-translate-y-0.5 cursor-pointer h-full">
              ...
              {/* Skills */}
              <div className="mt-3 pt-3 border-t border-[#2E2854] flex gap-1.5 flex-wrap">
