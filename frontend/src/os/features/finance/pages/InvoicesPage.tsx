  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard label="Overdue"        value={`₹${(totalOverdue() / 1000).toFixed(0)}k`}   iconColor={totalOverdue() > 0 ? 'bg-red-100 text-red-600' : 'bg-[#151C2F] text-slate-300'} icon={<FileText className="w-5 h-5" />} />
      </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
          ...
          {STATUS_FILTERS.map(s => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-semibold capitalize transition-all ${
                statusFilter === s ? 'bg-blue-600 text-white' : 'bg-[#151C2F] border border-[#2E2854] text-slate-300 hover:border-blue-300'
              }`}
            >
              {s}
            </button>
        ...
        <span className="ml-auto text-sm text-slate-500">{visible.length} invoices</span>
      </div>
      ...
      <Card padding="none">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-[#2E2854]">
            {visible.map(inv => (
              ...
              >
                <td className="px-5 py-3.5 font-mono text-xs text-slate-500">{inv.number}</td>
                <td className="px-5 py-3.5 font-medium text-slate-200">{inv.client}</td>
                <td className="px-5 py-3.5 text-slate-500 text-xs">{inv.projectName.split(' ').slice(0, 2).join(' ')}</td>
                <td className="px-5 py-3.5 font-bold text-white">{fmt(inv.amount)}</td>
                <td className="px-5 py-3.5 text-slate-500 text-xs whitespace-nowrap">{fmtDate(inv.issueDate)}</td>
        ...
        {visible.length === 0 && (
          <div className="py-12 text-center text-sm text-slate-500">No invoices match your filters.</div>
        )}
      </Card>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <p className="text-sm font-medium text-slate-300">{fmtDate(selected.issueDate)}</p>
              </div>
              <div className="space-y-1 text-right">
                <p className="text-xs text-slate-500">Due</p>
                <p className="text-sm font-medium text-slate-300">{fmtDate(selected.dueDate)}</p>
              </div>
              <InlineSelect
            ...
            <div className="space-y-2">
              ...
              {selected.items.map((item, i) => (
                <div key={i} className="grid grid-cols-12 text-sm py-2 border-b border-[#2E2854] last:border-0">
                  ...
                  <span className="col-span-2 text-right font-semibold text-white">{fmt(item.amount)}</span>
                </div>
              ))}
            </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-xl font-bold text-white">{fmt(selected.amount)}</span>
            </div>
