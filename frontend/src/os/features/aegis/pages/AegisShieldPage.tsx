  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-end">
        <span className="text-xs text-[var(--os-text-2)]">{total} blocked attempts</span>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left border-b border-[var(--os-border)]">
                ...
                <th className="pb-2 text-xs text-[var(--os-text-2)] font-medium pr-4">Role</th>
                <th className="pb-2 text-xs text-[var(--os-text-2)] font-medium">User</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[var(--os-border)]">
              {rows.map((row: any) => (
                <tr key={row.id} className="hover:bg-[var(--os-surface-0)] transition-colors">
                  ...
                  <td className="py-2 text-xs text-[var(--os-text-2)] max-w-[120px] truncate">{row.userId ?? '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
