  ...
  return (
    <div className="space-y-4">
      <div className="bg-violet-900/20 border border-violet-500/20 rounded-xl p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400 leading-relaxed">
            Every action KIMMP took <strong className="text-slate-300">without being asked</strong> — scheduled loops,
      ...
      <div className="flex justify-end">
        <span className="text-xs text-slate-500">{total} autonomous actions recorded</span>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {rows.map((row: any) => (
            <div key={row.id} className="bg-white/[0.03] border border-violet-500/10 hover:border-violet-500/25 rounded-xl p-4 transition-all">
              <div className="flex items-start justify-between gap-4">
                  ...
                  {row.agentsRun?.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-1.5">
                      {row.agentsRun.map((a: string) => (
                        <span key={a} className="text-[10px] font-mono bg-white/5 border border-white/10 rounded px-1.5 py-0.5 text-slate-400">
                          {a}
                        </span>
                      ))}
                    </div>
                ...
                <div className="text-right flex-shrink-0">
                  ...
                  {row.durationMs != null && (
                    <p className="text-[10px] text-slate-600 mt-0.5">{(row.durationMs / 1000).toFixed(1)}s</p>
                  )}
                </div>
              </div>
            </div>
