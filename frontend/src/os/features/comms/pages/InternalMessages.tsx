            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
              {loading && (
                <div className="space-y-3 animate-pulse">
                  {[1,2,3].map(i => <div key={i} className={`h-12 rounded-2xl bg-slate-900/40 backdrop-blur-2xl saturate-200 shadow-[inset_0_1px_0_rgba(255,255,255,0.15),0_8px_32px_rgba(0,0,0,0.3)] ring-1 ring-white/10 ${i % 2 === 0 ? 'ml-auto w-3/5' : 'w-3/5'}`} />)}
                </div>
              )}
              {!loading && thread.map(msg => {
                ...
                return (
                  <div key={msg.id} className={`flex gap-2.5 max-w-[80%] ${isMe ? 'ml-auto flex-row-reverse' : ''}`}>
                    ...
                    <div>
                      <div className={`rounded-2xl px-4 py-2.5 ${isMe ? 'bg-os-blue text-white rounded-tr-sm' : 'bg-slate-900/40 backdrop-blur-2xl saturate-200 shadow-[inset_0_1px_0_rgba(255,255,255,0.15),0_8px_32px_rgba(0,0,0,0.3)] ring-1 ring-white/10 text-slate-200 rounded-tl-sm'}`}>
            ...
            <div className="border-t border-white/10 border-t-white/20 px-4 py-3 flex-shrink-0">
              <div className="flex items-end gap-2">
                <textarea
                  ...
                  className="flex-1 resize-none border border-white/10 border-t-white/20 rounded-xl px-3 py-2.5 text-sm text-slate-300 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-blue-500/30 focus:border-blue-400"
                />
