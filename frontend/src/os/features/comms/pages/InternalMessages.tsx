        ...
        {selectedConv ? (
          <>
            ...
            <div className="flex items-center gap-3 px-5 py-4 border-b border-[var(--os-border)] flex-shrink-0">
              ...
              <div>
                <p className="text-sm font-semibold text-[var(--os-text-1)]">{selectedConv.user.name}</p>
                <p className="text-xs text-[var(--os-text-2)] capitalize">{selectedConv.user.role?.toLowerCase()}</p>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
              {loading && (
                <div className="space-y-3 animate-pulse">
                  {[1,2,3].map(i => <div key={i} className={`h-12 rounded-2xl bg-[var(--os-surface-0)] ${i % 2 === 0 ? 'ml-auto w-3/5' : 'w-3/5'}`} />)}
                </div>
              )}
              {!loading && thread.map(msg => {
                ...
                return (
                  <div key={msg.id} className={`flex gap-2.5 max-w-[80%] ${isMe ? 'ml-auto flex-row-reverse' : ''}`}>
                    ...
                    <div>
                      <div className={`rounded-2xl px-4 py-2.5 ${isMe ? 'bg-[#579bfc] text-white rounded-tr-sm' : 'bg-[var(--os-surface-0)] border border-[var(--os-border)] text-[var(--os-text-1)] rounded-tl-sm'}`}>
                        <p className="text-sm leading-relaxed">{msg.content}</p>
                      </div>
                      <p className={`text-[10px] text-[var(--os-text-2)] mt-1 ${isMe ? 'text-right' : ''}`}>
              ...
              {!loading && thread.length === 0 && (
                <p className="text-center text-xs text-[var(--os-text-2)] py-10">Start the conversation.</p>
              )}
              <div ref={bottomRef} />
            </div>
          ...
          <div className="flex flex-col items-center justify-center h-full text-center px-8">
            <div className="w-12 h-12 rounded-2xl bg-[var(--os-surface-0)] border border-[var(--os-border)] flex items-center justify-center mb-3">
              <MessageSquare className="w-5 h-5 text-[var(--os-text-2)]" />
            </div>
            <p className="text-sm font-semibold text-[var(--os-text-1)]">Select a conversation</p>
            <p className="text-xs text-[var(--os-text-2)] mt-1">Internal team messages</p>
          </div>
        )}
      </div>
    </div>
