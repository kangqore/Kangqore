...
export function ConversationList({
  ...
  return (
    <div className="flex flex-col h-full border-r border-[var(--os-border)]">
      {/* Search */}
      <div className="p-3 border-b border-[var(--os-border)] flex-shrink-0">
            ...
            {[1,2,3].map(i => (
              <div key={i} className="flex items-center gap-3 animate-pulse">
                ...
                <div className="flex-1 space-y-1.5">
                  ...
                  <div className="h-2.5 bg-[var(--os-surface-0)] rounded w-full" />
                </div>
              </div>
            ))}
          </div>
        ...
        {!loading && filtered.map(c => {
          ...
          return (
            <button
              ...
              className={cn(
                'w-full flex items-start gap-3 px-4 py-3.5 text-left transition-colors hover:bg-[var(--os-surface-0)] border-b border-[var(--os-border)]',
            ...
            >
              <div className="relative flex-shrink-0">
                ...
                {unread > 0 && (
                  <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-[#579bfc] text-white text-[9px] font-bold flex items-center justify-center">
                    {unread}
                  </span>
                )}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline justify-between gap-1 mb-0.5">
                  <p className={cn('text-xs font-semibold truncate', isSelected ? 'text-[#579bfc]' : 'text-[var(--os-text-1)]')}>
                    {user.name}
                  </p>
                  <span className="text-[10px] text-[var(--os-text-2)] flex-shrink-0">{fmtTime(time)}</span>
                </div>
                {user.company && (
                  <p className="text-[10px] text-[var(--os-text-2)] truncate mb-0.5">{user.company}</p>
                )}
                <p className={cn('text-[11px] truncate', unread > 0 ? 'text-[var(--os-text-1)] font-medium' : 'text-[var(--os-text-2)]')}>
                  {preview}
                </p>
              </div>
            </button>
