...
const Core: React.FC<WidgetProps> = ({ viewModel }) => {
    ...
    return (
        <div>
            <h3>Connectors</h3>
            <div className="focus-card"><span className="count">{entries.length}</span><label>Connected Systems</label></div>
            {entries.slice(0,5).map(([k,v],i)=>(
                <div key={i} className="mission-item">
                    <span className="mission-goal">{k}</span>
                    <span className="mission-status" style={{color:v==='OPERATIONAL'?'var(--os-success)':'var(--os-warning)'}}>{v==='OPERATIONAL'?'ON':'OFF'}</span>
                </div>
            ))}
        </div>
    );
