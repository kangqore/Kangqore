...
export function Topbar() {
  ...
  return (
    <header className="flex-shrink-0 h-[60px] bg-[#151C2F] border-b border-[#2E2854] flex items-center justify-between px-4 z-10">
      {/* App Launcher & Breadcrumbs */}
      <div className="flex items-center gap-4 flex-1">
        <Tooltip content="App Launcher" side="bottom">
          <button className="flex items-center justify-center w-9 h-9 rounded-lg text-slate-500 hover:bg-[#0F172A] hover:text-slate-300 transition-colors">
      ...
      <div className="flex-1 flex justify-center">
        <button
          ...
          className={cn(
            'relative h-9 w-full max-w-[400px] rounded-full border border-[#2E2854] bg-[#0F172A]',
            'flex items-center gap-2 pl-9 pr-3 text-sm text-slate-500',
            'hover:border-[#4ab6d4]/60 hover:bg-[#151C2F] transition-all duration-150 text-left'
          )}
        >
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 pointer-events-none" />
          Search anything…
          <kbd className="ml-auto text-[10px] text-slate-500 font-mono hidden lg:block bg-[#151C2F] px-1 border border-[#2E2854] rounded">⌘K</kbd>
        </button>
      </div>
      ...
      <div className="flex items-center gap-3 flex-1 justify-end">
        ...
        <div className="relative" ref={newRef}>
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-gradient-to-r from-[#2564ea] to-[#4ab6d4] text-white text-sm font-medium hover:opacity-90 transition-opacity"
          >
            <Plus className="w-3.5 h-3.5" />
            New
          </button>
          {newOpen && (
            <div className="absolute right-0 top-full mt-1.5 w-52 bg-[#151C2F] border border-[#2E2854] rounded-xl shadow-lg py-1 z-50">
              {NEW_ACTIONS.map(a => {
                const Icon = a.icon
                return (
                  <button
                    ...
                    className="flex items-center gap-2.5 w-full px-3 py-2 text-sm text-slate-300 hover:bg-[#0F172A] hover:text-white transition-colors text-left"
                  >
        ...
        <Tooltip content="Notifications" side="bottom">
          <button
            ...
            className="relative h-9 w-9 rounded-full flex items-center justify-center text-slate-500 border border-[#2E2854] hover:bg-[#0F172A] transition-colors"
          >
        ...
        <DropdownRoot>
          <DropdownTrigger asChild>
            <button className="flex items-center gap-2.5 rounded-full pl-1 pr-3 py-1 hover:bg-[#0F172A] transition-colors border border-transparent hover:border-[#2E2854]">
          ...
          <DropdownPortal>
            <DropdownContent align="end" sideOffset={8} className="z-50 min-w-[200px] bg-[#151C2F] border border-[#2E2854] rounded-xl shadow-lg p-1 animate-in fade-in-0 zoom-in-95 duration-150">
              <div className="px-3 py-2.5 border-b border-[#2E2854] mb-1">
                <p className="text-sm font-semibold text-white">{displayName}</p>
                <p className="text-xs text-slate-500 truncate">{displayEmail}</p>
                <Badge variant="brand" size="sm" className="mt-2 w-max">{displayRole}</Badge>
              </div>
              <DropdownItem className="flex items-center gap-2.5 px-2.5 py-2 text-sm text-slate-300 rounded-lg cursor-default outline-none hover:bg-[#0F172A] hover:text-white">
