...
const DashboardLayout = ({ children, role, title, subtitle, headerActions }) => {
  ...
  const { data: unreadData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL || ''}/api/notifications/unread-count`, {
  ...
  const { data: notificationsData } = useQuery({
    ...
    queryFn: async () => {
      const token = localStorage.getItem('token');
      const res = await axios.get(`${import.meta.env.VITE_BACKEND_URL || ''}/api/notifications?limit=5`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      return res.data;
    },
