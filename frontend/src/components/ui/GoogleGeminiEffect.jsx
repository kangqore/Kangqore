...
const PATH_COLORS = [
  "#FFB7C5", // Row 0: Rose/Pink
  "#FFDDB7", // Row 1: Amber/Gold
  "#B1C5FF", // Row 2: Soft Blue
  "#4FABFF", // Row 3: Cyan
  "#076EFF", // Row 4: Electric Blue
];
    ...
    <div className={cn("absolute inset-0 w-full h-full pointer-events-none z-20 overflow-visible", className)}>
      ...
      >
        <defs>
          ...
          <filter id="geminiGlowHighlight" x="-30%" y="-30%" width="160%" height="160%">
        ...
        {PATH_DATA.map((d, i) => {
          ...
          return (
            <g key={i}>
              ...
              {/* Authentic Organic Aceternity SVG Wave Path */}
              <motion.path
                d={d}
                stroke={color}
                ...
                filter={isHovered ? "url(#geminiGlowHighlight)" : undefined}
              />

              {/* Animated Action Potential Pulses (Data Flowing Left -> Right) */}
              <motion.path
                d={d}
                stroke="#ffffff"
                strokeWidth={strokeWidth + 1}
                strokeDasharray={isHovered ? "50 150" : "30 200"}
                fill="none"
                ...
                filter="url(#geminiGlowHighlight)"
              />
            </g>
          );
        })}
