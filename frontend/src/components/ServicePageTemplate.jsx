...
const ServicePageTemplate = ({ service, department }) => {
  ...
  const faqs = service.customFAQs || [
    { question: `What is ${service?.name || 'this service'} and how can it benefit my organization?`, answer: `${service?.name || 'This service'} helps organizations ${service.description?.toLowerCase() || 'achieve digital transformation'}. By implementing ${(service?.name || '').toLowerCase()}, you can improve efficiency, reduce costs, and gain competitive advantage.` },
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-20 text-left max-w-4xl">
            <div className="flex items-center gap-4 mb-8">
              <div className="h-[1.5px] w-16 bg-brand-blue"></div>
              <span className="text-[10px] font-black text-brand-blue uppercase tracking-[0.3em]">The Stack</span>
            </div>
            <h2 className="text-5xl lg:text-7xl font-bold text-gray-900 dark:text-white mb-10 font-display tracking-tight leading-[0.95]">
              {service.technologiesTitle ? (
                service.technologiesTitle === 'Tools & Technologies We Excel In' ? (
                  <>Tools & <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#2564ea] to-[#4ab6d4] italic">Technologies</span> We Excel In.</>
                ) : (
                  <>
                    {service.technologiesTitle.split(' ').slice(0, -1).join(' ')}{' '}
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#2564ea] to-[#4ab6d4] italic">{service.technologiesTitle.split(' ').slice(-1)}</span>
                  </>
                )
              ) : (
                <>Tools & <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#2564ea] to-[#4ab6d4] italic">Technologies</span> We Excel In.</>
            ...
            <div className={`w-24 h-1.5 bg-gradient-to-r from-[#2564ea] to-[#4ab6d4] rounded-full mb-10 opacity-20`}></div>
            <div className="text-2xl text-gray-500 leading-relaxed font-light max-w-2xl">
              {service.technologiesDescription || 'A model-agnostic, enterprise-grade technology stack purpose-built for autonomous, governed AI systems.'}
            </div>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-16">
            {Array.isArray(technologies) && technologies.length > 0 && typeof technologies[0] === 'object' && technologies[0].category ? (
              technologies.map((group, groupIdx) => (
                <div key={groupIdx} className="flex flex-col">
                  {/* Category Header */}
                  <div className="pb-4 mb-6 flex items-center gap-3">
                    <div className="w-2 h-2 rounded-full bg-gradient-to-r from-[#2564ea] to-[#4ab6d4]"></div>
                    <h3 className="text-sm font-bold text-gray-400 tracking-widest uppercase">{group.category}</h3>
                  </div>
                  ...
                  <ul className="space-y-4">
                    {group.items.map((tech, techIdx) => (
                      <li 
                        key={techIdx} 
                        className="flex items-center gap-3 text-xl lg:text-2xl font-semibold text-[#1D1D1F] tracking-tight hover:text-[#0071E3] transition-colors duration-300 cursor-default"
                      >
                        {typeof tech === 'object' && tech.logo && (
                          <img src={tech.logo} alt="" className="w-6 h-6 object-contain flex-shrink-0" loading="lazy" onError={(e) => { e.target.style.display = 'none'; }} />
                        )}
              ...
              <div className="col-span-full">
                 <div className="border-t border-gray-200 pt-8 flex flex-wrap gap-x-12 gap-y-6">
                  {technologies.map((tech, index) => (
                    <span 
                      key={index} 
                      className="text-3xl lg:text-5xl font-semibold text-[#1D1D1F] tracking-tighter hover:text-[#0071E3] transition-colors duration-300 cursor-default"
                    >
